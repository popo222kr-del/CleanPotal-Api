...
// LOT 번호 형식: "1{code}{yyMMdd}{일별순번:3자리}P" (예: 1SS260816003P).
// - 앞 "1"/끝 "P"는 고정 문자.
...
// - 일별순번은 코드별이 아니라 "그 날짜 전체"에서 공유된다("그날(YYMMDD) 안에서만 초기화") - 그래서
...
//
// MAX(LotNumber)+1 방식은 절대 사용하지 않는다 (CLAUDE.md 7번 / 절대 금지사항 4번) - 순번은
// SequenceCounter 가 원자적 UPDATE 로만 올린다.
public class LotNumberGenerator : ILotNumberGenerator
{
    private readonly ApplicationDbContext _context;

    public LotNumberGenerator(ApplicationDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<string> NextAsync(string code, CancellationToken cancellationToken = default)
    {
        ...
        var newValue = await SequenceCounter.NextAsync(_context, $"LOT-{dateKey}", cancellationToken);
