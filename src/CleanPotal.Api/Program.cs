...
if (useSqlite)
{
    ...
    builder.Services.AddDbContext<CleanPotalDbContext>(opt => opt
        ...
        // 시작할 때 Migrate() 는 부르지 않는다 — 스키마는 DatabaseSchemaInitializer/SchemaUpgrader 가 추가만 해서 맞춘다.
        // 마이그레이션은 2026-07 에서 멈춰 있어 누가 손으로 Migrate()(dotnet ef database update 등)를 부르면
        // EF 9+ 는 "모델이 마이그레이션보다 앞서 있다" 를 예외로 다룬다. 그 경고만 무시해 둔다.
