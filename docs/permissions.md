...
MES 는 관리자 전용 도구가 아니라 **전 직원이 권한을 받아 함께 쓰는** 시스템이다.
그래서 다른 영역에 얹지 않고 자기 영역을 갖는다 — 현장 점검(field)에 얹어 두면
"MES 만 쓰는 사람"도, "현장 점검은 주되 MES 는 빼는 사람"도 만들 수 없다.

| 등급 | 뜻 |
|---|---|
| 0 | MES 메뉴가 사이드바에서 사라진다 |
| 1 (기본) | LOT 현황·이력 조회 |
| 2 | 전산등록·공정 이동 등 작업 |

**기본값이 1(조회)인 이유** — 전 직원이 쓰는 시스템이라 관리자가 한 명씩 열어 줄 때까지
잠겨 있으면 안 된다. `Users.AccessMes` 컬럼은 `DEFAULT 1` 로 추가되므로, 이미 등록된 사용자도
서버가 한 번 뜨는 순간 곧바로 MES 를 볼 수 있다. **작업(등급 2)은 관리자가 올려 준다** —
사용자 계정 관리 → 권한 매트릭스의 `MES (생산관리)` 칸, 또는 역할 프리셋(현장 작업자·현장 리더·Office).

정책 이름은 `ViewMes`(1) / `EditMes`(2) 이고, 판정은 다른 영역과 똑같이 매 요청 DB 를 본다
(`DbPermissionHandler`) — 등급을 바꾸면 재로그인 없이 바로 반영된다.
