...
| 운영 | `http://10.10.10.119:8713` | IIS `Cleanjueon` | 연두 **운영 서버** |

- 사이드바 로고 아래 띠(`테스트 서버 d2f7b57 · 09-26 09:30`), 로그인·휴대폰 화면의 로고 옆 배지, 브라우저 탭 제목(`[테스트] …`)으로 어느 서버·어느 빌드인지 구분한다(`/api/about`, `PortalAbout`).
