...
웹에서 올리는 사진·파일(체크시트, BROKEN, 주간보고, 기타세정 현황, 주간세정 현황, 생산팀 요청사항)은 모두 **첨부 보관소** 한 곳에 파일로 저장된다. DB 에는 위치와 이름만 한 줄 남는다.
...
<Storage:AttachmentsPath>\
  ...
  주간세정\2026-09\20260925_091500_(업체)_작업.jpg   ← 업체 관리에서 "주간세정" 표시한 업체
...
하는 일:
1. **DB 칸 안의 사진(base64)을 파일로 꺼낸다** — 기타세정·주간세정 현황, 생산팀 요청사항(최근까지 DB 에 통째로 저장, 주간세정 업체 것은 주간세정 폴더로), 주간보고·BROKEN 의 옛 기록. 칸에는 `att:번호|이름|종류` 만 남는다. 파일 이름은 그 기록의 작성일 기준.
